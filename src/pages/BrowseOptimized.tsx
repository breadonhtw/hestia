...
import { useInfiniteArtisans } from "@/hooks/useInfiniteArtisans";
import { InfiniteScrollTrigger } from "@/components/InfiniteScrollTrigger";
import { Creator } from "@/types/creator";

/**
 * Optimized Browse Page with Infinite Scroll
 * 
 * Optimizations inspired by X (Twitter) and Instagram:
 * 1. Infinite scroll - loads content progressively
 * 2. Progressive image loading - blur-up technique
 * 3. Optimized caching - reduces API calls
 * 4. Content-visibility CSS - renders only visible items
 * 5. Prefetching - loads next page before user reaches bottom
 */
const BrowseOptimized = () => {
  const {
    data,
    fetchNextPage,
    hasNextPage,
  ...
  // Filter creators (memoized for performance)
  const filteredCreators = useMemo(() => {
    return creators.filter((creator) => {
      ...
      const artisan = allArtisans?.find((a) => a.id === creator.id);
  ...
  }, [creators, selectedCrafts, selectedLocation, acceptingOrders, openForCommissions, newlyJoined, allArtisans]);
        ...
        <div className="container mx-auto px-4 lg:px-8 py-12">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <main className="flex-1">
              ...
              {/* Creator Grid with Infinite Scroll */}
              {isLoading ? (
...
export default BrowseOptimized;
